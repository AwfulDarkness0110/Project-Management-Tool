...
function checkIfUserIsInProject(userId, projectId, res, requiresOwner) {
  return new Promise(async (resolve, reject) => {
    const projectData = await project.findById(projectId).catch((err) => {
      console.log(err);
    });
    ...
    if (requiresOwner === true) {
      ...
      if (
        projectData.owner.includes(userId) ||
        projectData.users.includes(userId)
      ) {
        return resolve(projectData);
      } else {
        res.status(401).send({
...
exports.addUser = async (req, res) => {
  ...
  if (!foundUser) {
    ...
    return res.status(404).send({
      message:
        "We couldn't find this email. Are you sure it is registered on our platform?",
    });
  }
    ...
    .then((data) => {
      if (!data) {
        ...
        if (data.owner[0] && data.owner[0].equals(foundUser._id)) {
          ...
          return res
            .status(400)
            .send({ message: "This user is the owner of the project" });
        }
  ...
  if (!error) {
    if (req.headers.role === "OWNER" || req.headers.role === "ADMIN") {
      ...
      res.status(401).json({
        message:
          "You dont have permissions to do this. Please contact the owner of the project.",
      });
    }
  }
};
      ...
      .updateOne(
        {
          ...
          "userRoles.userId": mongoose.Types.ObjectId(req.body.userId),
        },
...
exports.removeUser = (req, res) => {
  checkIfUserIsInProject(req.body.userMakingRequestId, req.body.projectId, res)
    .then(async () => {
      const foundUser = await db.users
        .findOne({ email: req.body.userEmail })
        .catch((err) => {
...
exports.addUserToTask = async (req, res) => {
  ...
  await project
    .updateOne(
      ...
      {
        $push: {
          "columns.$[].tasks.$[taskfield].asignee": mongoose.Types.ObjectId(
            foundUser._id
          ),
        },
      },
      {
        arrayFilters: [
          { "taskfield._id": mongoose.Types.ObjectId(req.body.taskId) },
        ],
      }
    )
    .then((result) => {
...
exports.update = (req, res) => {
  ...
  checkIfUserIsInProject(userId, id, res)
    .then(() => {
      if (!req.body) {
        return res.status(400).send({
          message:
            "Data to update can not be empty!" + " - " + console.log(req.body),
        });
      }
...
exports.updateTask = (req, res) => {
  ...
  project
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || "Some error occurred while Editing task-",
      });
    });
};
// Update a TASK by the ID - QUICK-EDIT
...
exports.moveTask = async (req, res) => {
  ...
  checkIfUserIsInProject(req.body.userId, req.body.projectId, res).then(
    async () => {
      var taskData;
      await project
        .find({ "columns.tasks._id": mongoose.Types.ObjectId(id) })
        .then(async (result) => {
    ...
    .then(() => {
      project
        .update(
          { "columns._id": mongoose.Types.ObjectId(id) },
          {
            $pull: { columns: { _id: mongoose.Types.ObjectId(id) } },
          }
        )
        .then((data) => {
          if (!data) {
            ...
            res.send({ message: "column was deleted successfully!" + `${id}` });
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  project
    ...
    .then((data) => {
      if (!data) {
        ...
        res.send({
          message: "Column name was updated successfully! " + `${id}`,
        });
    })
    .catch((err) => {
      res.status(500).send({
